              ...
              <div className="space-y-1 text-sm">
                ...
                {currentMetrics.accuracy != null && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono font-medium">{formatPercent(currentMetrics.accuracy)}</span>
                  </div>
                )}
                {currentMetrics.val_loss != null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Val Loss:</span>
                    <span className="font-mono font-medium">{formatValue(currentMetrics.val_loss)}</span>
                  </div>
                )}
                {currentMetrics.val_accuracy != null && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Val Accuracy:</span>
                    <span className="font-mono font-medium">{formatPercent(currentMetrics.val_accuracy)}</span>
                  </div>
                )}
              </div>
            ) : (
